      ...
      get "#{@@mount_path}/:name" do
        ...
        ops.each do |path, op_routes|
          operations = op_routes.map do |route|
            ...
            # Prepare data for `:consumes` and `:produces` keys customization
            ...
            # Customize `:consumes` key
            consumes_content_types = [all_content_types[format]] # only use one that current format parses
            operation[:consumes] = consumes_content_types if route.route_method != 'GET' && consumes_content_types && !consumes_content_types.empty?

            # Customize `:produces` key
            ...
            # Allows to add common custom headers as parameters for a set of routes
            # Passed with `namespace_stackable(:swagger_request_headers, value)` at some namespace
            ...
            # Passed with `namespace_stackable(:swagger_options, value)` at some namespace
            global_options = endpoint.namespace_stackable(:swagger_options)
            global_options.each { |e| operation.merge!(e) }

            # Passed with `:swagger` key on `desc` call for route description
